...
import com.zhoubo07.bannerlib.view.CBLoopViewPager;
...
public class CBLoopScaleHelper {
    ...
    public void attachToRecyclerView(final  CBLoopViewPager mRecyclerView){
        attachToRecyclerView(mRecyclerView,false,0);
    }

    public void attachToRecyclerView(final CBLoopViewPager mRecyclerView,boolean isSnapLeft,int snapLeftMargin) {
        if (mRecyclerView == null) {
            return;
        }

        this.isSnapLeft = isSnapLeft;
        this.snapLeftMargin = snapLeftMargin;
        if(isSnapLeft){
            mPagerSnapHelper = new PagerLeftSnapHelper(snapLeftMargin);
        }else {
...
//                        setCurrentItem(position);
                ...
                if(onPageChangeListener != null) {
                    ...
                    if(count != 0 && newState == 0){
//                        RecyclerView.LayoutManager layoutManager = recyclerView.getLayoutManager();
//                        View itemView = layoutManager.getChildAt(position);
//                        onPageChangeListener.onPageSelected(itemView,position % count);
...
//                setCurrentItem(mFirstItemPos);
                if(isSnapLeft)scrollToPositionFirst(mFirstItemPos);
                else setCurrentItem(mFirstItemPos);
    ...
    public void scrollToPositionFirst(int pos) {
        if (mRecyclerView == null) {
            return;
        }
        ((LinearLayoutManager) mRecyclerView.getLayoutManager()).
                scrollToPositionWithOffset(pos,snapLeftMargin);
        mRecyclerView.post(new Runnable() {
            @Override
            public void run() {
                onScrolledChangedCallback();
    ...
    public int getRealCurrentItem() {
        CBPageAdapter adapter = (CBPageAdapter) mRecyclerView.getAdapter();
        int count = adapter.getRealItemCount();
        return count==0?0:getCurrentItem()%count;
    }
